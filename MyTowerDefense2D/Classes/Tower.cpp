...
CCArray* MutilTower::getMutilFireEnemys(){
    ...
    }else
    {
        ...
        if (int count =arr->count()<=3) {
            CCArray *retArr = CCArray::createWithCapacity(count);
...
void MutilTower::towerLogic1(float dt){
    ...
    if (enemyArr==NULL) {
        this->setenemys(this->getMutilFireEnemys());
        return;
    }else{
        if (enemyArr->data==NULL|| enemyArr->count()==0) {
            return;
        }
        CCObject *obj=NULL;
